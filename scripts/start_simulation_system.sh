...
start_service worker2 env USE_OLLAMA=false RAG_URL="$RAG_URL" "$PYTHON" -m workers.gpu_worker --id 2 --master-id 1 --port 9003
start_service worker3 env USE_OLLAMA=false RAG_URL="$RAG_URL" "$PYTHON" -m workers.gpu_worker --id 3 --master-id 1 --port 9004
...
start_service master1 env WORKER_REQUEST_TIMEOUT=60 "$PYTHON" -m master.scheduler --master-id 1 --port 8002 --workers 2:9003 3:9004

wait_for_http "http://127.0.0.1:8001/health" "Master 0"
wait_for_http "http://127.0.0.1:8002/health" "Master 1"

start_service load_balancer env LB_MASTER_TIMEOUT=60 "$PYTHON" -m lb.load_balancer
wait_for_http "$LB_URL/health" "Load balancer"
set_strategy round_robin

echo "Simulation system is running."
show_logs_hint

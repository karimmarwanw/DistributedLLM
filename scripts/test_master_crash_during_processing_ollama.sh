...
echo "Forcing Master 0 to crash while Worker 0 is processing its next scheduled request..."
curl -fsS -X POST "http://127.0.0.1:8001/simulate/fail-during-next-schedule?delay=1&reason=master%200%20crashed%20while%20worker%20was%20processing"
echo

echo "Sending request through load balancer. Expected final master_id: 1 and worker_id: 1"
"$PYTHON" -m client.load_generator \
  --users 1 \
  --timeout 400 \
  --query "Write a long detailed explanation about fault tolerance in distributed systems." \
  --full-answer \
  --max-tokens 1200
show_logs_hint

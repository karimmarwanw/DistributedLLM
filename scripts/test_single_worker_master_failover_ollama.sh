...
echo "Forcing the only worker under Master 0 to fail during its next request..."
curl -fsS -X POST "http://127.0.0.1:9001/simulate/fail-next?delay=2&reason=only%20worker%20under%20master%200%20failed"
echo

echo "Sending request through load balancer."
echo "Expected behavior: Master 0 stays alive, reports no successful worker, and load balancer retries Master 1."
echo "Expected final response: master_id=1 and worker_id=1."
...
echo
echo "Master 0 health after failure:"
